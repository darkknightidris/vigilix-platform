...
export default async function VulnDetailPage({ params }: { params: Promise<{ id: string, vulnId: string }> }) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="p-4 bg-gray-900 rounded-xl border border-gray-800">
          ...
          <p className="text-white font-medium">{vuln.cvss_score || "—"}</p>
        </div>
        <div className="p-4 bg-gray-900 rounded-xl border border-gray-800">
          <p className="text-gray-400 text-xs mb-1">Deadline</p>
          <p className="text-white font-medium">
            {vuln.due_date ? new Date(vuln.due_date).toLocaleDateString("id-ID") : "—"}
          </p>
        </div>
      </div>
      ...
      {vuln.description && (
        <div className="p-6 bg-gray-900 rounded-xl border border-gray-800">
          <h2 className="text-white font-semibold mb-3">Deskripsi</h2>
      ...
      {vuln.steps_to_reproduce && (
        <div className="p-6 bg-gray-900 rounded-xl border border-gray-800">
          <h2 className="text-white font-semibold mb-3">Langkah Reproduksi</h2>
          <pre className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap font-mono bg-gray-800 p-4 rounded-lg">{vuln.steps_to_reproduce}</pre>
        </div>
      )}
